    ...
    .map(
      (resident) => `
        <tr>
          ...
          <td>${escapeHtml(getCondoName(resident.condoId))}</td>
          <td>${escapeHtml(resident.unit)}</td>
          <td>${resident.days} dias</td>
          <td>${currency.format(resident.amount)}</td>
          <td>${nextChannel(resident.days)}</td>
        </tr>
      `,
    )
    .join("");
    ...
    .map(
      (resident) => `
        <tr>
          ...
          <td>${escapeHtml(getCondoName(resident.condoId))}</td>
          <td>${escapeHtml(resident.unit)}</td>
          <td><span class="pill ${resident.status === "overdue" ? "danger" : ""}">${resident.status === "paid" ? "Adimplente" : "Inadimplente"}</span></td>
          <td>${currency.format(resident.amount)}</td>
        </tr>
      `,
    )
